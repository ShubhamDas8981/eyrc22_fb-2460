defmodule Task1aSumOfSubsets do
       @moduledoc """
         ...
         def valid_sum(matrix_of_sum) do
              Enum.filter(List.flatten(matrix_of_sum),fn(x)->(x !="na") end)
         end
         ...
         def sum_of_one(array_of_digits, sum_val) do
              ...
              if array_of_digits == [] || sum_val == 0 do
                     []
              else
                     [head | tail]=array_of_digits
              Enum.filter(combine([head | tail]), fn(x) -> Enum.sum(x) == sum_val end)
              end
         end
         def combine([]) do
              []
         ...
         def sum_of_all(array_of_digits,matrix_of_sum) do
              Map.new(valid_sum(matrix_of_sum), fn x -> { x, sum_of_one(array_of_digits,x)} end)
         end
       end
